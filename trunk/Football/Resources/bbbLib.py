"""
 ...
 Contains general functions, that have no script dependencies, so be used with other scripts.

 ChangeLog:
 22/10/06 - Created.
 15/11/06 - Change: fetchURL() - use safe_url
            Change: cleanHTML() - to use regex not find
            Change: tweeks to many other funcs. (eg decodeEntities())
            New: urlTextToASCII(), unicodeToAscii()
 06/12/06 - Change: DialogSelect()
 06/01/07 - Added searchRegEx(), findAllRegEx(), isImageURL(), isRSSLink(),
            isHTMLLink(), ViewTextFile(),listDir()
 01-02-07 - Added KEY_BUTTON_<x> definitions,
            Changed DialogSelect() to also take list of lists [[lbl1,lbl2],[lbl1,lbl2]...]
            Added ConfigHelper() - funcs to help each script with ConfigParser
            Added ConfigCommonSections - class holding common config sections
            Added isMC360()
13-02-07    Added createCache()
05-03-07	Changed DialogSelect() to take a banner - displayed above list
            Added safeFilename - os path safe chars
11-04-07    Changed: Few tweaks. Added RSSParser2()
...
import xbmc, xbmcgui
xbmc.output("loading module: " + __name__ + " Version: " + __version__ + " Date: " + __date__)
import sys,os.path
import os, re, unicodedata, traceback, urllib, sgmllib, ConfigParser
from threading import Thread
from string import strip, replace, find, rjust, translate
...
ACTION_RECORD			= 2010  # record button on remote - assigned in keymap.xml
...
def updateGlobals(basePath=''):
	...
	global BTN_A_FILENAME, BTN_B_FILENAME, BTN_X_FILENAME, BTN_Y_FILENAME, BTN_WHITE_FILENAME
...
def isMC360(skinName):
	state = (find(skinName,'360') >= 0)
	debug("isMC360() " +str(state))
	return state
...
class DialogSelect(xbmcgui.WindowDialog):
	...
	def setup(self, title='',width=430, height=450, xpos=-1, ypos=-1, textColor='0xFFFFFFFF', \
			  imageWidth=0,imageHeight=20, itemHeight=20, rows=0,
			  useY=False, useX=False, isDelete=False, \
			  panel='', banner=''):
		...
		titleY = ypos + 10
		bannerY = titleY + titleH
		listY = bannerY + bannerH
		...
		if not self.isDelete:
			self.menuCL = xbmcgui.ControlList(xpos+offsetListX, listY, \
											...
											itemHeight=itemHeight, itemTextXOffset=0, space=0)
		else:
			self.menuCL = xbmcgui.ControlList(xpos+offsetListX, listY, \
											listW, listH, textColor=textColor, \
											imageWidth=imageWidth, imageHeight=imageHeight, \
											itemHeight=itemHeight, buttonFocusTexture=LIST_DELETE_FILENAME, \
											itemTextXOffset=0, space=0)
	...
	def setMenu(self, menu, icons=[]):
		...
		if menu:
			for idx in range(len(menu)):
				...
				elif isinstance(opt, list) or isinstance(opt, tuple):	# option is itself a list [lbl1, lbl2]
					l1 = opt[0].strip()
					try:
						l2 = opt[1].strip()
					except:
						l2 = ''
					self.menuCL.addItem(xbmcgui.ListItem(l1, l2, thumbnailImage=img))
				else:
					self.menuCL.addItem(xbmcgui.ListItem(opt.strip(), '', thumbnailImage=img))
	...
	def ask(self, menu, selectIdx=-1, icons=[]):
		...
		self.doModal()
		self.selectedPos -= 1
		debug("< DialogSelect.ask() selectedPos: " + str(self.selectedPos))
...
class AdjustOverscan(xbmcgui.Window):
	def __init__(self):
		...
		setResolution(self)
		self.isTL = True
		self.backg = xbmcgui.ControlImage(0,0, REZ_W, REZ_H, BACKG_BLANK_FILENAME)
		self.info = xbmcgui.ControlTextBox(150, 150, 400, 300, 'font12')
		self.tlCB = xbmcgui.ControlButton(0, 0, 0, 0, '', focusTexture=OVERSCAN_TL_FILENAME)
		self.brCB = xbmcgui.ControlButton(0, 0, 0, 0, '', focusTexture=OVERSCAN_BR_FILENAME)
	...
	def draw(self):
		...
		if self.isTL:
			...
			self.tlCB.setLabel('')
			self.brCB.setLabel('Adjust')
			self.setFocus(self.brCB)

		xbmcgui.unlock()

	##############################################################################################
	def ask(self, top=30, left=30, bottom=30, right=30):
		debug("> ask()")
		# OVERSCAN
		self.overscanT = top
		self.overscanL = left
...
class TextBoxDialog(xbmcgui.WindowDialog):
	...
	def setupDisplay(self):
		...
		dialogW = REZ_W - 100
		dialogH = REZ_H - 100
		xpos = int((REZ_W /2) - (dialogW /2))
		ypos = int((REZ_H /2) - (dialogH /2)) +10

		try:
			self.addControl(xbmcgui.ControlImage(xpos, ypos, dialogW, dialogH, DIALOG_PANEL))
		except: pass

		xpos += 20
		ypos += 20
		self.titleCL = xbmcgui.ControlLabel(xpos, ypos, dialogW-35, 30, '','font14', "0xFFFFFF00", alignment=XBFONT_CENTER_X)
		self.addControl(self.titleCL)
		ypos += 30
		self.descCTB = xbmcgui.ControlTextBox(xpos, ypos, dialogW-35, dialogH-70, 'font12', '0xFFFFFF66')
		self.addControl(self.descCTB)
		debug("< setupDisplay()")


	def ask(self, file="", title="", text=""):
...
class ConfigHelper:
	def __init__(self, filename=''):
		debug("> ConfigHelper.__init__")
		debug("filename="+filename)

		self.filename = filename

		# Config actions
		ConfigHelper.MODE_SECTIONS = 'ls'			# list of sections
		ConfigHelper.MODE_OPTIONS = 'lo'			# list options in the given section
		ConfigHelper.MODE_ITEMS = 'li'				# list of items (name, value pairs) in the given section
		ConfigHelper.MODE_INIT_OPTION = 'i'			# read, init option to a default value if not exists
		ConfigHelper.MODE_INIT_SECTION = 'is'		# read, init option to a default value if not exists
		ConfigHelper.MODE_READ = 'r'				# read in the given section
		ConfigHelper.MODE_WRITE = 'w'				# write in the given section
		ConfigHelper.MODE_REMOVE_SECTION = 'rs'		# remove section
		ConfigHelper.MODE_REMOVE_OPTION = 'ro'		# remove option in the given section
		ConfigHelper.MODE_HAS_SECTION ='hs'			# check if section exists

		ConfigHelper.VALUE_YES = 'YES'
		ConfigHelper.VALUE_NO = 'NO'
	...
	def save(self):
		try:
			...
			success = False
		debug("ConfigHelper.save() success=" + str(success))
	...
	def load(self):
		try:
			...
			success = False
		debug("ConfigHelper.load() success=" + str(success))
	...
	def action(self, section='', option='', value='', mode=None):
		...
		debug("< ConfigHelper.action() retValue=" + str(retValue))
		return retValue
	...
	def __setOption(self, section, option, value):
		try:
			...
			if isinstance(value, bool):
				if value:
					value = ConfigHelper.VALUE_YES
				else:
					value = ConfigHelper.VALUE_NO
			self.cp.set(section, option, str(value))
			return True
		except:
			handleException("Config WRITE Error: " + os.path.basename(self.filename))
			return False


#############################################################################################################
# Used as convienience values for each script that uses SYSTEM & DISPLAY config sections
class ConfigCommonSections:
	def __init__(self):

		ConfigCommonSections.filename = DIR_SYSTEM + 'Config.dat'

		# define sections/options/values specific to this scripts needs

		# section keys
		ConfigCommonSections.SECTION_SYSTEM = "SYSTEM"
		ConfigCommonSections.SECTION_DISPLAY = "DISPLAY"

		# DISPLAY keys
		ConfigCommonSections.KEY_DISPLAY_OVERSCAN_LEFT = 'overscan_left'
		ConfigCommonSections.KEY_DISPLAY_OVERSCAN_RIGHT = 'overscan_right'
		ConfigCommonSections.KEY_DISPLAY_OVERSCAN_TOP = 'overscan_top'
		ConfigCommonSections.KEY_DISPLAY_OVERSCAN_BOTTOM = 'overscan_bottom'
		ConfigCommonSections.KEY_DISPLAY_SKIN = 'skin'

		# DISPLAY default values
		ConfigCommonSections.VALUE_DISPLAY_SKIN_PMIII = 'PMIII'
		ConfigCommonSections.VALUE_DISPLAY_SKIN_MC360 = 'MC360'
		ConfigCommonSections.VALUE_DISPLAY_OVERSCAN_LEFT_MC360 = 99
		ConfigCommonSections.VALUE_DISPLAY_OVERSCAN_LEFT = 30
		ConfigCommonSections.VALUE_DISPLAY_OVERSCAN_RIGHT = 30
		ConfigCommonSections.VALUE_DISPLAY_OVERSCAN_TOP = 15
		ConfigCommonSections.VALUE_DISPLAY_OVERSCAN_BOTTOM = 15

	###############################################################################################################
	# init common DISPLAY section values
	###############################################################################################################
	def initSectionDisplay(self):
		debug("> CCS.initSectionDisplay()")
		items = {}
		if IS_MC360:
			items[self.KEY_DISPLAY_OVERSCAN_LEFT] = self.VALUE_DISPLAY_OVERSCAN_LEFT_MC360
			items[self.KEY_DISPLAY_SKIN] = self.VALUE_DISPLAY_SKIN_MC360
		else:
			items[self.KEY_DISPLAY_OVERSCAN_LEFT] = self.VALUE_DISPLAY_OVERSCAN_LEFT
			items[self.KEY_DISPLAY_SKIN] = self.VALUE_DISPLAY_SKIN_PMIII
		items[self.KEY_DISPLAY_OVERSCAN_RIGHT] = self.VALUE_DISPLAY_OVERSCAN_RIGHT
		items[self.KEY_DISPLAY_OVERSCAN_TOP] = self.VALUE_DISPLAY_OVERSCAN_TOP
		items[self.KEY_DISPLAY_OVERSCAN_BOTTOM] = self.VALUE_DISPLAY_OVERSCAN_BOTTOM
		debug("< CCS.initSectionDisplay()")
		return items
	...
	# loads associated skin config (if exists) for whatever the current xbmc skin is DIR_SKIN
	###############################################################################################################
	def loadSkinConfig(self, defaultDisplaySection):
		debug("> CCS.loadSkinConfig()")

		# save skinName
		skinFN = DIR_SKIN + '\\skin.dat'
		defaultDisplaySection[self.KEY_DISPLAY_SKIN] = SKIN_NAME
		if not fileExist(skinFN):
			debug("missing skin config file=" + skinFN)
		else:
			try:
				debug("loading skin config file=" + skinFN)
...
#############################################################################################################
def createCache(cacheDir):
	debug("createCache() cacheDir = " + cacheDir)
	try:
		os.mkdir(cacheDir)
	except: pass
