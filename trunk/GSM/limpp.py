...
TODO:
    ...
    Imporve (make correct?) the adaptive filtering
'''
################################################################################
''' Class: Image_Error '''
################################################################################
...
################################################################################
''' Class: Fake_BMP_image '''
################################################################################
class Fake_BMP_image(Indexed_image,BGR_image,RLE_image):
    def __init__(self,addr=0,size=None,file=None,process=True):
        self.init()
        if not size:
            size = ospath_getsize(file)
...
class RGBA_to_PNG(SaveImage_base):
    ...
    def __init__(self,rgba_data,file,type='RGBA8',filter=0,compression=6):
        ...
        try:
            ...
            print 'Error With Height and/or Width'
            return
...
def Get_image(mipmap=0,addr=0,size=None,file=None,process=True,options=None):
    ...
    try:
        if type == 'XPR0':
            ...
            #is_image_bmp
            return Fake_BMP_image(addr=addr,size=size,file=file,process=process)
