...
class window(xbmcgui.Window):
	...
	def setupControls(self):
		...
		guibuilder.GUIBuilder(self, SkinPath + skin + '\\skin.xml', SkinPath + skin, __title__, __line1__, dialog, pct, debug=True)
		#self.setFocus(self.controls[10])
	...
	def getConfigValues(self):
		try:
			...
			if (not path.exists(location)): location = DefaultPath + 'default_' + curURL_Location +'.xml'
...
#		self.saveDefaults()
	...
	def addMaps(self, silent = False):
		...
		try:
			xbmcgui.lock()
			for frame in range(10): self.controls[50 + frame].setVisible(False)
			
			if (result):
				for frame in range(mapFiles):
					self.controls[50 + frame].setImage('%s%s%d%s' % (fpath, self.imageName[5], frame, ext[5],))
			else:
				self.mapFiles = 0
				self.controls[50 + self.mapFiles].setImage(ImagePath + '\\na.gif')
			self.controls[50].setVisible(True)
			
			if (ext[0] != 'n/a' and result): self.controls[200].setImage('%s%s%s' % (fpath, self.imageName[0], ext[0],))
			else: self.controls[200].setImage('')
			
			if (ext[1] != 'n/a' and result): self.controls[201].setImage('%s%s%s' % (fpath, self.imageName[1], ext[1],), ck[1])
			else: self.controls[201].setImage('')
			
			if (ext[2] != 'n/a' and result): self.controls[202].setImage('%s%s%s' % (fpath, self.imageName[2], ext[2],), ck[2])
			else: self.controls[202].setImage('')
			
			if (ext[3] != 'n/a' and result): self.controls[203].setImage('%s%s%s' % (fpath, self.imageName[3], ext[3],), ck[3])
			else: self.controls[203].setImage('')

			#Grid maybe re-enable it (only used for doppler)
			#if (ext[4] != 'n/a' and result): self.controls[204].setImage('%s%s%s' % (fpath, self.imageName[4], ext[4],), ck[4])
			#else: self.controls[204].setImage('')
			...
			if (feed):
				...
				for description in feed:
					...
					while (len(description) > 255):
						...
						self.controls[20].addLabel(description[:i] + ' (cont)')
						description = description[i + 1:]
					self.controls[20].addLabel(description)
			if (not silent):
				dlg.update(100)
				dlg.close()
			...
			if (IMAGES[5] != ''):
				...
				for url in range(5):
					...
					if (IMAGES[url] != ''):
						...
						if ((not path.exists(fpath + self.imageName[url] + ext[url])) or url == 0 or not result):
							# download image
							if (not debug):
	...
	def convertOverlays(self, result, fpath, ext, ck):
		try:
			for i in range(1, 5):
				if (ext[i] != 'n/a'):
					if ((not path.exists(fpath + self.imageName[i] + '.png')) or (not result)):
	...
	def switchFrame(self, x):
		...
		self.controls[50 + self.FRAME].setVisible(True)
		self.controls[50 + tmpFrame].setVisible(False)
		self.setButton4()


	def setButtonsStatus(self):
		self.controls[10].setLabel(self.category[self.CATEGORY])
		self.controls[11].setLabel(self.level[self.LEVEL])
		self.controls[12].setLabel(self.type_title[self.TYPE])
		self.controls[13].setLabel(self.animate[self.ANIMATE])
		self.controls[13].setEnabled(self.MAPFILES > 1)
		self.setButton4()


	def setButton4(self):
		self.controls[14].setLabel('Frame:  (%d/%d)' % (self.FRAME + (1 * (self.MAPFILES > 0)),self.MAPFILES,))
		self.controls[14].setEnabled(self.MAPFILES > 1 and not self.ANIMATE)
	...
	def playerChk(self):
		self.controls[300].setVisible(xbmc.getCondVisibility(self.visibility[300]))
		self.controls[301].setVisible(xbmc.getCondVisibility(self.visibility[301]))
		self.controls[302].setVisible(xbmc.getCondVisibility(self.visibility[302]))
		self.controls[303].setVisible(xbmc.getCondVisibility(self.visibility[303]))
	...
	def onAction(self, action):
		if (action == ACTION_PARENT_DIR or action == ACTION_PREVIOUS_MENU): self.exitScript()
			...
			if (c == self.controls[10]): self.switchCategory(x)
			elif (c == self.controls[11]): self.switchLevel(x)
			elif (c == self.controls[12]): self.switchType(x)
			elif (c == self.controls[13]): self.switchAnimate()
			elif (c == self.controls[14]): self.switchFrame(x)


	def onControl(self, control):
		if (not self.INITIALIZING):
			if (control == self.controls[10] or control == self.controls[11] or control == self.controls[12]): self.addMaps()
			elif (control == self.controls[13]): self.switchAnimate()
			elif (control == self.controls[14]): self.switchFrame(1)
			elif (control == self.controls[15]): self.setAsDefault()
