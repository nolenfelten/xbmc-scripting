...
class ListingData:
	...
	def config(self, reset=False):
		...
		self.IP = config.action(CONFIG_SECTION, CONFIG_OPT_IP)
		self.PORT = config.action(CONFIG_SECTION, CONFIG_OPT_PORT)
		...
		if not self.IP:
			...
			if self.IP and self.IP != origIP:
				config.action(CONFIG_SECTION, CONFIG_OPT_IP, self.IP, mode=ConfigHelper.MODE_WRITE)
		...
		if not self.PORT:
			...
			if self.PORT and self.PORT != origPORT:
				config.action(CONFIG_SECTION, CONFIG_OPT_PORT, self.PORT, mode=ConfigHelper.MODE_WRITE)
