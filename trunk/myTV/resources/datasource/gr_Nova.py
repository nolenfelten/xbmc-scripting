...
class ListingData:
	...
	def config(self, reset=False):
		...
		self.lang = mytvGlobals.config.action(CONFIG_SECTION, CONFIG_KEY_LANG)
		origLang = self.lang 
		...
		if reset or not self.lang:
			...
			if selectedPos >= 0:
				...
				mytvGlobals.config.action(CONFIG_SECTION, CONFIG_KEY_LANG, self.lang, mode=ConfigHelper.MODE_WRITE)
