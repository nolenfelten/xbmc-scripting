...
class BaseScriptDialog( xbmcgui.WindowXMLDialog ):
    def __init__( self, xmlFile = '', resourcePath = '', defaultName = 'Default', forceFallback = False ):
        '''
            Even though the arguments above show defaults of empty strings for 
                xmlFile and resourcePath, all subclasses must pass a proper 
                xml filename and the correct resource path:

                    BaseScriptDialog( xmlFile, resourcePath )

            The code to automatically generate these arguments has been left 
                here for demonstration purposes, and in the hopes that in the 
                future, subclasses can use the intended syntax:

                    BaseScriptDialog()

            Note: see docstring for BaseScriptWindow for further discussion.
        '''
        if not len( xmlFile ):
            xmlFile = 'script-%s-window.xml' % common.scriptname.replace( ' ', '_' )
        if not len( resourcePath ):
