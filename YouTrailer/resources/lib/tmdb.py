...
class MovieDb:
    ...
    def searchimdb(self, imdbid):
        url = config['urls']['movie.imdbLookup'] % (imdbid)
        etree = XmlHandler(url).getEt()
        search_results = SearchResults()
        for cur_result in etree.find("movies").findall("movie"):
            cur_movie = self._parseMovie(cur_result)
    ...
    def getinfo(self, tmdbid):
        url = config['urls']['movie.getInfo'] % (tmdbid)
        etree = XmlHandler(url).getEt()
        search_results = SearchResults()
        for cur_result in etree.find("movies").findall("movie"):
            cur_movie = self._parseMovie(cur_result)
