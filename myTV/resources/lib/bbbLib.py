...
def makeScriptDataDir():
	try:
		scriptPath = xbmc.translatePath("/".join(["T:","script_data", __scriptname__]))
...
def installPlugin(pluginType, scriptname, checkOnly=True, msg=""):
	...
	try:
		...
		copyToPath = xbmc.translatePath( "/".join( ["Q:", "plugins", pluginType, name] ) )
...
#######################################################################################################################    
def getTodayDate(secs=0):
    if secs:
        today = int(time.strftime("%Y%m%d", time.localtime(secs)))
    else:
        today = int(time.strftime("%Y%m%d",time.localtime()))
    debug("getTodayDate() from secs=%d = %s" % (secs, today))
    return today
