...
class ConfigSaveProgramme:
	...
	def reset(self):
		...
		if origCardType != cardType:
			mytvGlobals.config.action(self.CONFIG_SECTION, mode=ConfigHelper.MODE_REMOVE_SECTION)
			mytvGlobals.config.action(self.CONFIG_SECTION, self.KEY_CARD_TYPE, cardType, mode=ConfigHelper.MODE_WRITE)
	...
	def getValue(self, key):
		value = mytvGlobals.config.action(self.CONFIG_SECTION, key)
		if value == None:
			value = ''
		return value
