...
#							print progList[-1]
	...
	def config(self, reset=False):
		...
		self.USER = config.action(CONFIG_SECTION, KEY_USER)
		self.PWD = config.action(CONFIG_SECTION, KEY_PASS)
		dst = config.action(CONFIG_SECTION, KEY_DST)
		if dst not in (True, False):
			dst = False
		self.DST = config.configHelper.boolToYesNo(dst)
...
############################################################################################################
#def createFilesFromXMLTV(filename, chID, GMToffset, requiredDate):
#	debug( "> createFilesFromXMLTV() chID=%s GMToffset=%s requiredDate=%s" % (chID, GMToffset,requiredDate))
#	GMToffsetSecs = (GMToffset * 60) * 60
#	progList = []
#
#	try:
#		channelProgs = []
...
#			for progID, progDateTime, progDur in channelProgs:
#
#				# create date in reverse. ie 2005-06-30 -> 20050630
#				reverseDate = int(progDateTime[:4] + progDateTime[5:7] + progDateTime[-2:])
#
#				# create data file if doesnt exist
#				if reverseDate != requiredDate:
#					continue
#
#				# calc programme start/end time in secs since epoch based on programme date
#				startTimeSecs = time.mktime(time.strptime(progDateTime,"%Y-%m-%dT%H:%M:%SZ"))
...
#						} )
#	except:
#		handleException("createFilesFromXMLTV()")
#
#	debug( "< createFilesFromXMLTV() prog count=%s" % len(progList))
#	return progList
