...
class ListingData:
	...
	def config(self, reset=False):
		...
		if reset:
			...
			self.country_code = mytvGlobals.config.action(CONFIG_SECTION, CONFIG_OPT)
		...
		mytvGlobals.config.action(CONFIG_SECTION, CONFIG_OPT, self.country_code, mode=ConfigHelper.MODE_WRITE)
		self.setup()
